...
class ExtendedMemoryManager(BaseModel):
    model: Optional[Model] = None
    agent_id: Optional[str] = None  # Added agent_id to MemoryManager
    ...
    def add_memory(self, memory: str) -> str:
        ...
        """
        logger.debug(f"ExtendedMemoryManager try to add memory")
        try:
            if self.db:
                self.db.upsert_memory(
                    ExtendedMemoryRow(agent_id =self.agent_id,user_id=self.user_id, memory=Memory(memory=memory, input=self.input_message).to_dict())
                )
    ...
    def update_memory(self, id: str, memory: str) -> str:
        ...
        """
        logger.debug(f"ExtendedMemoryManager try to update memory")
        try:
            if self.db:
                self.db.upsert_memory(
                    MemoryRow(
