...
def get_async_playground_router(
    ...
    @playground_router.get("/status2")
    async def playground_status2():
    ...
    @playground_router.post("/agents/{agent_id}/edit")
    async def edit_agent(agent_id: str, body: dict = Body(...)):
        """
        API endpoint để chỉnh sửa thông tin của một agent.
        """
        logger.debug(f"EditAgentRequest: {agent_id} {body}")
        # Tìm agent dựa trên agent_id
        agent = get_agent_by_id(agent_id, agents)
        if agent is None:
            logger.info(f"Agent {agent_id} not found. Creating a new agent.")
            try:
                agent_id = str(uuid.uuid4())
            
                new_agent = Agent(agent_id=agent_id, **body)
                agents.append(new_agent)  # Thêm agent mới vào danh sách agents
                logger.info(f"Created new agent with ID {agent_id}.")
                return JSONResponse(content={"message": f"Agent {agent_id} created successfully."}, status_code=201)
            except Exception as e:
        ...
        try:
            for key, value in body.items():
                if hasattr(agent, key):
                    ...
                    logger.info(f"Updated {key} for agent {agent_id} to {value}")
                else:
                    logger.warning(f"Attribute {key} does not exist on agent {agent_id}")

            return JSONResponse(content={"message": f"Agent {agent_id} updated successfully."})
        except Exception as e:
